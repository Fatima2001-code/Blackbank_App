...
namespace BankApplication
{
    public partial class Monthly : Form
    {
        ...
        private void button2_Click(object sender, EventArgs e)
        {
            ...
            string query = "SELECT * FROM [TRANSACTION] WHERE (Acc_id_sender = @AccID OR Acc_id_receiver = @AccID) AND MONTH(Tra_date) = @Month";
            SqlCommand cmd = new SqlCommand(query, conn);
            cmd.Parameters.AddWithValue("@AccID", text_AccID.Text);
            cmd.Parameters.AddWithValue("@Month", numericUpDown1.Value);
